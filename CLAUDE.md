# Claude Code Project Instructions — envie-kotlin

## Project Overview

This is **envie-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`envie`](https://crates.io/crates/envie).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/envie/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/envie>
